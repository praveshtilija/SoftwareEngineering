...
public class DBHelper {
    ...
    public static Cursor queryLastItemInserted(SQLiteDatabase db){
        String query = "SELECT * FROM " + GLMDataBase.ITEMS_TABLE + " WHERE " + GLMDataBase.ITEMS_COLS[0] +
                " = (SELECT MAX(" + GLMDataBase.ITEMS_COLS[0] + ") FROM "+ GLMDataBase.ITEMS_TABLE +")";
    ...
    public static boolean deleteCustomItem(SQLiteDatabase db, int id){
        long result = db.delete(GLMDataBase.ITEMS_TABLE, GLMDataBase.ITEMS_COLS[0] + "=" + id, null);
        return result > 0;
    }
}

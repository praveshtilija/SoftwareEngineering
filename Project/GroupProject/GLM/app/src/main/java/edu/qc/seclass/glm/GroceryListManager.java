...
public class GroceryListManager {
    ...
    /**
     * TODO: Requirements: The user creates a new item in the List Manager
     */
    public Item createItem(Item itemToInsert){

        ContentValues values = Converter.itemToDB(itemToInsert);
        boolean success = DBHelper.insertToItems(db.getWritableDatabase(), values);

        if (!success) return null;

        Cursor cursor = DBHelper.queryLastItemInserted(db.getReadableDatabase());
        List<Item> item = Converter.DBtoItems(cursor);

        if (item.size() != 1) return null;

        /* This is the actual GroceryList that is saved into the DB */
        Item lastAdded = item.get(0);

        /* Saves the grocery list to our local mapped data structures for easy lookup */
        this.defaultItems.put(lastAdded.getItemId(), lastAdded);



        this.getTypeLookup().get(lastAdded.getType()).add(lastAdded);
...
//        this.groceryListNames.add(lastAdded.getName());
    ...
    public void removeCustomItem(Item item) {
        ...
        boolean result = DBHelper.deleteCustomItem(this.db.getWritableDatabase(), item.getItemId());

        if (result) {
            this.getTypeLookup().get(item.getType()).remove(item);
        }
